...
<div class="container py-4">
  ...
  <div class="card card-custom p-4 mb-4">
    ...
    <div class="mb-3">
      ...
      {{ $submission->customer->Fname ?? 'N/A' }} {{ $submission->customer->Lname ?? '' }}
    </div>

    <div class="mb-3">
      <strong>Category:</strong>
      {{ $submission->category->categoryname ?? 'N/A' }}
    </div>
    ...
    <div class="mb-3 d-flex align-items-center gap-2">
      ...
      <form action="{{ route('staff.submission.status', $submission->SubmissionID) }}" method="POST" class="d-inline-block">
        ...
        <select name="status" class="form-select w-auto" onchange="this.form.submit()">
          ...
          <option value="In Progress"
                  @selected($submission->status == 'In Progress')
                  @disabled($submission->status != 'Pending')>
            In Progress
          </option>
          <option value="Resolved"
                  @selected($submission->status == 'Resolved')
      ...
      @if($submission->status === 'Resolved')
        <form action="{{ route('staff.submission.close', $submission->SubmissionID) }}" method="POST">
          @csrf
          <button type="submit"
                  class="clsbtn btn-danger"
                  onclick="return confirm('Are you sure you want to close this submission?')">
            Close
          </button>
        </form>
      @endif
    ...
    <div class="mb-3">
      ...
      {{ \Carbon\Carbon::parse($submission->dateSubmitted)->format('M d, Y') }}
    </div>
      ...
      <form action="{{ route('staff.submission.comment', $submission->SubmissionID) }}" method="POST">
        ...
        <div class="mb-3">
          <input 
            ...
            name="action_taken"
            class="form-control"
            placeholder="Optional action taken">
        </div>

        <div class="d-flex gap-2">
          <a href="{{ route('staff.dashboard') }}" class="btn btn-dashboards">
            Return
          </a>
    ...
    @if(!$submission->is_deleted)
      <form action="{{ route('staff.submission.delete', $submission->SubmissionID) }}"
            method="POST"
            class="d-inline"
            onsubmit="return confirm('Are you sure you want to mark this submission as deleted?');">
        @csrf
        @method('DELETE')
    ...
    @else
      <span class="text-muted align-self-center">
        Deleted. Admin can permanently remove.
      </span>
    @endif
